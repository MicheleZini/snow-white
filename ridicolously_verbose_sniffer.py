  ...
  while True:
    ...
    if eth_protocol == 8 :
        ...
        if protocol == 6 :
            ...
            print " | / from %15s  port %s%5i%s                                \ |" % (s_addr,srccolor,source_port,defcolor)
            print " | | to   %15s  port %s%5i%s                                | |" % (d_addr,dstcolor,dest_port,defcolor)

            # - https ----------------------
            if source_port == 443 or dest_port == 443:
		print " | |  %s_ https? ____________________________________________________%s  | |"  %  (pldcolor,defcolor)
		print " | | / payload size: %5i                                         \ | |" % data_size

            # - http -----------------------
            elif source_port == 80 or dest_port == 80:
                print " | |  %s_ http? _____________________________________________________%s  | |"  %  (pldcolor,defcolor)
		print " | | / payload size: %5i                                         \ | |" % data_size

            # - ssh -----------------------
            elif source_port == 22 or dest_port == 22:
                print " | |  %s_ ssh? ______________________________________________________%s  | |"  %  (pldcolor,defcolor)
		print " | | / payload size: %5i                                         \ | |" % data_size 

            # - http -----------------------
            elif source_port == 21 or dest_port == 21:
                print " | |  %s_ telnet? ___________________________________________________%s  | |"  %  (pldcolor,defcolor)
		print " | | / payload size: %5i                                         \ | |" % data_size

	    # - other ----------------------
            else:
                print " | |  %s_ unknow ____________________________________________________%s  | |"  %  (pldcolor,defcolor)
		print " | | / payload size: %5i                                         \ | |" % data_size
